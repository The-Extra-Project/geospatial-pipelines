...
import os
from lib.colmap.scripts.python.read_write_model import read_model
...
## apply on the dataset: http://www.scan-net.org or "https://www.eth3d.net/data/"
# ## parses the dataset on which reconstruction is to be run 
# def load_datasets(dataset_name: str):
#     check_call("curl", f"{baseurl}{dataset_name}.zip", "-o", f"{dataset_name}.zip")
#     check_call(["unzip", f"{dataset_name}.zip"])

# def extract_metadata_colmap(dataset_name):
#     os.chdir()
#     pass
from neuralangelo.dataset import base
from utils import cameras

## only to show the images (without the other video spaces).
ImageFile.LOAD_TRUNCATED_IMAGES = True

class Dataset(base.Dataset):
    
    def __init__(self, cfg, is_inference=False):
        super().__init__(self,is_inference=is_inference, is_test=False)
        cfg_data = cfg.data
