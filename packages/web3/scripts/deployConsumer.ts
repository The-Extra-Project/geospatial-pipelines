...
task("deploy-consumer", "deploys first the consumer contract and then sets the addresses and params")
...
.setAction(async (taskArgs: any) => {
    ...
    const donIdBytes32 = hre.ethers.utils.formatBytes32String(functionsRouter["donId"])
    ...
    console.log(
      `\nWaiting ${network_params[network.name].confirmations} blocks for transaction ${
        consumerContract.deployTransaction.hash
      } to be confirmed...`
    )
    await consumerContract.deployTransaction.wait(network_params[network.name].confirmations)
